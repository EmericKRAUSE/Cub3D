...
/*   Updated: 2025/04/15 15:22:26 by nidionis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (game->map.tab)
		clean_exit(game, "[map] Error: Multiple maps", ERR_MULTIPLE_MAPS, line);
	else
...
void	invalid_line(t_game *game, char *line)
{
	...
	if (line)
		free(line);
	line = NULL;
	clean_exit(game, "Error: Invalid line", ERR_INVALID_LINE, line);
}
