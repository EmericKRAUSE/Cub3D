...
/*   Updated: 2025/04/15 15:25:45 by nidionis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	prepend_line(t_game *game, char ***map, char *line)
{
	...
	if (!new_map)
		clean_exit(game, "Error: malloc failed (ft_calloc)", ERR_MALLOC, line);
	new_map[0] = line;
	while ((*map)[++i - 1])
		new_map[i] = (*map)[i - 1];
	free(*map);
...
void	add_top_bottom_wall(t_game *game, char ***map, char c)
{
	...
	if (!line)
		clean_exit(game, "Error: add wall (ft_calloc)", ERR_MALLOC, line);
